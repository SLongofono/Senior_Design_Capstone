...
define rv_csr
    if $argc == 0
        ...
        printf "    mcounteren  : 0x%08lX", p->mmu->sim->procs[0]->state.mcounteren
        printf "    frm         : 0x%016lX\n", p->mmu->sim->procs[0]->state.frm
        printf "    mscratch    : 0x%016lX", p->mmu->sim->procs[0]->state.mscratch
        printf "    scounteren  : 0x%08lX", p->mmu->sim->procs[0]->state.scounteren
        ...
        if $argc == 1
            ...
            if $arg0 == $rv_mcounteren
                printf "    mcounteren  : 0x%08lX\n", p->mmu->sim->procs[0]->state.mcounteren
                printf "           [ 0] =>  CY  => %i\n", p->mmu->sim->procs[0]->state.mcounteren << (31 -  0) >> (32 - 1)
                printf "           [ 1] =>  TM  => %i\n", p->mmu->sim->procs[0]->state.mcounteren << (31 -  1) >> (32 - 1)
                printf "           [ 2] =>  IR  => %i\n", p->mmu->sim->procs[0]->state.mcounteren << (31 -  2) >> (32 - 1)
            end
            if $arg0 == $rv_scounteren
                printf "    scounteren  : 0x%08lX\n", p->mmu->sim->procs[0]->state.scounteren
                printf "           [ 0] =>  CY  => %i\n", p->mmu->sim->procs[0]->state.scounteren << (31 -  0) >> (32 - 1)
                printf "           [ 1] =>  TM  => %i\n", p->mmu->sim->procs[0]->state.scounteren << (31 -  1) >> (32 - 1)
                printf "           [ 2] =>  IR  => %i\n", p->mmu->sim->procs[0]->state.scounteren << (31 -  2) >> (32 - 1)
            end
            if $arg0 == $rv_sepc
...
define rv_virt
    if $argc != 2
        p "usage: rv_virt address bytes"
    else
        set $rv_virt_address = $arg0
        set $rv_virt_num_bytes = $arg1
        
        set $rv_virt_physical = p->mmu->translate($rv_virt_address, LOAD)
        
        rv_mem $rv_virt_physical $rv_virt_num_bytes
    end
end
...
define rv_dis
    if $argc == 0
        ...
        set $rv_dis_physical = p->mmu->translate(pc, FETCH)
        set $rv_dis_address_start = $rv_dis_physical
        set $rv_dis_address_end = $rv_dis_physical + $rv_dis_num_bytes - 1
        set $rv_dis_error = 0
        set $rv_dis_offset = 0
        set $rv_dis_byte = 0
...
define rv_bug_run
    ...
    b csrrw.cc:15 if pc == 0xffffffff8008cd30
        commands
            set logging overwrite on
            set logging on
            set logging redirect on
            set pagination off
            set p->mmu->sim->procs[0]->state.XPR.data[3] = 0xffffffff8037f238
            continue
        end
    set logging off
    set pagination on
...
rv_step
r -m512 --isa=RV64IMA --pc=0x90000000 /home/babypaw/riscv/riscv-tools/riscv-pk/build/bbl /home/babypaw/riscv/riscv-linux/vmlinux
set $ram_base = &((p->mmu->sim->mems[0].second).data[0x0])
restore /home/babypaw/riscv/gdb/linux_kernel_bad_2.bin binary $ram_base
set $rom_base = &((p->mmu->sim->mems[0].second).data[0x10000000])
restore /home/babypaw/riscv/gdb/rom_image.bin binary $rom_base
