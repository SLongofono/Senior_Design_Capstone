...
2. "ise_flow.bat" command need to run from "ISE Design Suite Command Prompt"
   to activate the license . Otherwise Xilinx environment variable need to set
   in order to run implementation properly.
...
2. Xilinx environment variable need to set in order to run implementation
   properly.

Windows/Linux OS:

3. On running "ise_flow.sh/ise_flow.bat" file, removes the XST report files
   first (if exist any on previous runs) and then does implementation of
   the design.

4. Running the "ise_flow.sh/ise_flow.bat" file creates the ise_flow_results
